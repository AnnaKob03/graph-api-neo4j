...
async def startup():
    try:
        with driver.session() as session:
            session.run("RETURN 1")  # Простой запрос для проверки подключения
        print("Успешное подключение к базе данных Neo4j.")
    except Exception as e:
        print(f"Ошибка подключения к базе данных Neo4j: {e}")
        raise

@app.get("/")
async def root():
    return {"message": "Welcome to the API. Please refer to the documentation at /docs"}
